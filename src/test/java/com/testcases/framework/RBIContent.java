...
@TestCaseName(name = "RBI Content Creation")
public class RBIContent extends BaseTestCase {
    WebDriver wd;
    ReUseableMethods md;

    @Test(retryAnalyzer = com.util.framework.ReTry.class)
    public void createRBIContent(){
        try{
            long startTime = System.currentTimeMillis();
            wd = getCustomDriver();
            report.addTest(getTestName("com.testcases.framework.RBIContent"));
            ...
            con.andThen(conSearch).andThen(createDuplicate)
                                  ...
                                  .andThen(updateOfferDescription)
                                  .andThen(updateOfferMoreInfo)
                                  .andThen(updateOfferHowToReDeem)
                                  .andThen(updateOfferUIPatterns)
                                  //.andThen(updateOfferRedemptionMethods)
                                  //.andThen(updateOfferShortCode)
                                  //.andThen(updateOfferPrice)
                                  //.andThen(rulesetDelete)
                                  //.andThen(rulesetAddAnd)
                                  //.andThen(option)
                                  .andThen(delete)
                                  .accept(wd);
